...
def test_all_pages_a11y_sweep(built_site: Path) -> None:
    ...
    More permissive than the per-page test (only blocks on issues that are
    nearly always real). Catches accidental regressions on auto-generated
    ...
    for pg in built_site.rglob("*.html"):
        ...
        if not scan.has_main:
            failures.append(f"{pg.relative_to(built_site)}: no <main>")
        if not scan.has_lang:
            failures.append(f"{pg.relative_to(built_site)}: no <html lang>")
        if scan.empty_links:
            failures.append(
                f"{pg.relative_to(built_site)}: {len(scan.empty_links)} empty link(s)"
            )
        if scan.imgs_without_alt:
            failures.append(
                f"{pg.relative_to(built_site)}: {len(scan.imgs_without_alt)} img(s) w/o alt"
            )
